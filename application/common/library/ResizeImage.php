...
class ResizeImage {
    ...
    {
        $this->config['impath'] = config('thumb.impath');
        $this->config['thumb_type'] = config('thumb.cut_type');
        $this->config['thumb_quality'] = 100;
        //         $this->config['thumb_quality'] = (config('thumb_quality') ? config('thumb_quality') : 100);
        $this->srcimg = $img;
        $this->scale = $scale;
        ...
        $this->dstimg = $dst_img;
        $image_info = @getimagesize($this->srcimg);
        if (!empty($image_info['mime']) && strpos($image_info['mime'], '/') !== false) {
            $this->type = ltrim(strstr($image_info['mime'], '/'), '/');
        } else {
    ...
    private function dst_img()
    {
        ...
        $this->relative_dstimg = str_replace(UPLOAD_PATH, '', $this->dstimg);
    }
}